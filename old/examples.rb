...
class Princess
  ...
  FUNCTIONS = {
    ...
    '>' => ->{ set_velocity_maybe_increase RIGHT },
    '<' => ->{ set_velocity_maybe_increase LEFT },
    '^' => ->{ set_velocity_maybe_increase UP },
    'v' => ->{ set_velocity_maybe_increase DOWN },
    '=' => ->{ push (pop == pop ? 1 : 0) },
    '!' => ->{ push pop.truthy? ? 0 : 1 },
    'N' => ->{ self[step!.tap { @dir = 0 }] },
    '+' => ->{ push pop(-2) + pop },
    "'" => ->{abort},
    '_' => ->{ push pop(-2) - pop },
    '*' => ->{ push pop(-2) * pop },
  ...
  def inspect = "Princess(pos=#@pos, dir=#@dir, stack=#@stack)"
...
Princess.new(<<EOS.lstrip, stack: [10]).play
-------------------------------v
Q . "_d_l_r_o_w_ _o_l_l_e_H_"_<<
0>>? 1 q
