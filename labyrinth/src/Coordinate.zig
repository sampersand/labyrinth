...
pub fn moveBy(coord: Coordinate, by: Vector) MoveError!Coordinate {
    return .{
        .x = std.math.cast(u32, @as(i64, coord.x) + @as(i64, by.x)) orelse return error.CoordinateOutOfBounds,
        .y = std.math.cast(u32, @as(i64, coord.y) + @as(i64, by.y)) orelse return error.CoordinateOutOfBounds,
    };
}
